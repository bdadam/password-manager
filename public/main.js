...
/***/ function(module, exports, __webpack_require__) {
	...
	Vue.component('vault-details', {
	    ...
	    created: function created() {
	        ...
	        this.store.subscribe(function () {
	            console.log('sub');
	            _this3.state = _this3.store.getState();
	            _this3.vault = _this3.state.vaults[_this3.vaultid];
	        });
	...
	Vue.component('vault-view', {
	    ...
	    data: function data() {
	        return { vault: null, password: '', decryptedSecrets: [] };
	    },
	
	    methods: {
	        initializeVault: function initializeVault(initPassword) {
	    ...
	    computed: {
	        ...
	        vaultLocked: function vaultLocked() {
	            return this.vault.hasPassword && true;
	        },
	        vaultOpen: function vaultOpen() {
	            return false;
	        },
	        decryptedSecrets: function decryptedSecrets() {
	            return [{ title: 'title1234', username: 'user1', password: 'password1', description: 'description', tags: ['asdf', 'qwe', 'sfsdf', 'abc'] }, { title: 'title1234', username: 'user2', password: 'password2', description: 'description', tags: ['asdf', 'qwe', 'sfsdf', 'abc'] }, { title: 'title1234', username: 'user3', password: 'password3', description: 'description', tags: ['asdf', 'qwe', 'sfsdf', 'abc'] }, { title: 'title1234', username: 'user4', password: 'password4', description: 'description', tags: ['asdf', 'qwe', 'sfsdf', 'abc'] }, { title: 'title1234', username: 'user5', password: 'password5', description: 'description', tags: ['asdf', 'qwe', 'sfsdf', 'abc'] }];
	        }
	    }
	});
	...
	var model = new Vue({
	    ...
	    data: (_data = {
	        ...
	        starting: true,
	        currentview: ''
	    }, _defineProperty(_data, 'store', _store2.default), _defineProperty(_data, 'view', ''), _defineProperty(_data, 'viewParams', {}), _data),
	    ...
	    methods: {
	        openView: function openView(view, params) {
	            this.view = view;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var vaultStore = (0, _redux.createStore)(function () {
	    var state = arguments.length <= 0 || arguments[0] === undefined ? { vaults: {}, passwords: {}, decryptedVaults: {} } : arguments[0];
	    var action = arguments[1];
	    ...
	    switch (action.type) {
	        ...
	        case 'unlock-vault':
	            ...
	            return Object.assign({}, state, {
	                passwords: Object.assign({}, state.passwords, _defineProperty({}, action.id, action.password)),
	                decryptedVaults: Object.assign({}, state.decryptedVaults, _defineProperty({}, action.id, state.vaults[action.id]))
	            });
	
	            var vault = state.vaults[action.id];
	            var pw = action.password;
	            var decrypt = function decrypt(x) {
	                return decodeURIComponent(x);
	            };
	            var decryptedSecrets = vault.encryptedSecrets.map(decrypt);
	
	            var vaults = Object.assign(state.vaults);
	            vaults[action.id].password = action.password;
	            vaults[action.id].decryptedSecrets = vaults[action.id].encryptedSecrets.map(decrypt);
	
	            return Object.assign({}, state, { vaults: vaults });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var before = __webpack_require__(17);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getISOYear = __webpack_require__(38)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var parse = __webpack_require__(29)
	var startOfYear = __webpack_require__(31)
...
/***/ function(module, exports) {
	...
	module.exports = "<prompt-vault-create v-if=\"prompt === 'vault-create'\" @cancel=closePrompt @vaultcreated=vaultcreated></prompt-vault-create><prompt-vault-passwordchange v-if=\"prompt === 'vault-passwordchange'\"></prompt-vault-passwordchange><prompt-vault-unlock v-if=\"prompt === 'vault-unlock'\" @cancel=closePrompt @vaultunlocked=vaultunlocked></prompt-vault-unlock><ul v-if=\"view === 'vault-list'\" class=vault-list><li v-for=\"(id, vault) in vaults\" class=vault-list-item><div class=status><svg class=icXon viewbox=\"0 0 32 32\" style=\"height: 32px; width: 22px;\"><use xlink:href=#icon-lock></use></svg>locked</div><a class=main href=\"/vaults/{{ id }}\" @xclick.prevent=openVault(id)><h3>{{ vault.name }}</h3><p style=\"font-size: 0.875rem; color: #999;\">{{ id }}</p></a><div class=actions><a class=btn-primary href=\"/vaults/{{ id }}\" @xclick.prevent=openVault(id)>Open vault</a> <button class=btn @click=removeVault(id)>Remove</button></div></li></ul><modal-window v-if=\"prompt === 'vault-unlock-password'\"><form @submit.prevent=openVaultWithPassword><p>Your vault is locked. Please provide your password to unlock it.</p><div class=form-row><label>Password</label> <input type=password v-model=promptedVaultPassword> <button>Unlock</button> <button @click.prevent=closePrompt>Cancel</button></div></form></modal-window><vault-details v-if=\"view === 'vault-details'\" :store=store :vaultid=params.vaultid></vault-details><div style=\"display: none;\"><svg id=icon-lock xmlns=http://www.w3.org/2000/svg xwidth=32 xheight=32 viewbox=\"0 0 22 32\"><path d=\"M18.5 14H18V8c0-3.308-2.692-6-6-6H8C4.692 2 2 4.692 2 8v6h-.5c-.825 0-1.5.675-1.5 1.5v15c0 .825.675 1.5 1.5 1.5h17c.825 0 1.5-.675 1.5-1.5v-15c0-.825-.675-1.5-1.5-1.5zM6 8c0-1.103.897-2 2-2h4c1.103 0 2 .897 2 2v6H6V8z\"></path></svg></div>"
